...
export const SearchModule = {
    ...
    filterCatalog(query) {
        ...
        return this.products.filter(p => {
            const name = (p.name || p.Title || p.title || p.Name || '').toLowerCase();
            const category = (p.category || p.Category || '').toLowerCase();
            const sku = (p.sku || p.SKU || p.Id || p.id || '').toLowerCase();
    ...
    showAutocomplete(query, inputElement) {
        ...
        if (matchedProducts.length > 0) {
            ...
            matchedProducts.forEach(p => {
                ...
                const formattedPrice = Utils.formatCurrency ? Utils.formatCurrency(productPrice) : ('₹' + productPrice.toLocaleString('en-IN'));

                html += `<a class="dropdown-item d-flex align-items-center gap-2 py-2" href="product.html?slug=${p.slug || p.Slug}">
                    <img src="${p.image_url_1 || p.image || p.Image || '404.webp'}" style="width: 30px; height: 30px; object-fit: contain;" alt="">
                    <div class="text-truncate">
                        <div class="fw-medium text-dark text-truncate fs-7">${p.name || p.Title || p.title || p.Name}</div>
                        <div class="text-primary fs-8">${formattedPrice}</div>
                    </div>
                </a>`;
            });
        }
    ...
    performSearch(query) {
        ...
        if (totalResults === 0) {
            resultsContainer.innerHTML = `<div class="col-12 text-center py-5">
        ...
        if (filteredProducts.length > 0) {
            ...
            outputHtml += filteredProducts.map(p => {
                ...
                return `
                    <div class="col">
                        <div class="card h-100 shadow-sm product-card border-0" style="width: 100%;">
                            <div class="product-img-container product-clickable text-center p-3 bg-white rounded-top" data-sku="${productSku}" style="cursor: pointer; height: 180px; display: flex; align-items: center; justify-content: center;">
                                <img src="${productImg}" class="img-fluid" alt="${productName}" style="max-height: 150px; object-fit: contain;" onerror="this.src='404.webp'">
                            </div>
                            <div class="card-body d-flex flex-column product-info bg-light rounded-bottom">
                                ...
                                <h5 class="card-title text-truncate fs-6" title="${productName}">${productName}</h5>
                                <p class="card-text text-primary fw-bold fs-5 mt-1">${formattedPrice}</p>
                                <div class="d-flex gap-2 mt-auto">
                                    ...
                                    <button class="btn btn-primary btn-sm w-50 add-to-cart-btn"
                                        ...
                                        data-image="${productImg}">Add to Cart</button>
                                </div>
                            </div>
                        </div>
                    </div>
